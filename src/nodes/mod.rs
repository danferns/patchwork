...
pub fn inline_port_circle(
    ...
    crate::app::draw_shaped_port(
        ui.painter(), rect.center(), 6.0, fill, border, 2.0, kind, is_wired,
    );
...
pub fn render_content(
    ...
    match node_type {
        ...
        NodeType::Display { history, history_max, scope_min, scope_max, scope_height, paused, display_color, label, auto_fit } =>
            display::render(ui, node_id, values, connections, history, history_max, scope_min, scope_max, scope_height, paused, display_color, label, auto_fit, port_positions, dragging_from),
        NodeType::Add | NodeType::Multiply => math::render(ui, node_id, values),
        NodeType::Math { formula, variables, result, error, .. } =>
            math_formula::render(ui, formula, variables, result, error, node_id, values, connections, port_positions, dragging_from),
        ...
        NodeType::Theme { dark_mode, accent, font_size, bg_color, text_color, window_bg, window_alpha, grid_color, grid_style, wire_style, wiggle_gravity, wiggle_range, wiggle_speed, rounding, spacing, use_hsl, wire_thickness, background_path } =>
            theme::render(ui, dark_mode, accent, font_size, bg_color, text_color, window_bg, window_alpha, grid_color, grid_style, wire_style, wiggle_gravity, wiggle_range, wiggle_speed, rounding, spacing, use_hsl, wire_thickness, background_path, node_id, values, connections, port_positions, dragging_from),
        ...
        NodeType::StringFormat { template, arg_count } =>
            string_format::render(ui, template, arg_count, node_id, values, connections, port_positions, dragging_from),
